...
const DetailScreen = ({ route }) => {
  ...
  const deleteComment = async (commentId) => {
    try {
      const response = await fetch(`http://192.168.0.108:3000/comments/${commentId}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
            ...
            if (item.trim() !== '') {
              return (
                <View>
                  ...
                  <Text style={styles.imageAuthor}>Autor zdjęcia: {castle.imageAuthor}</Text>
                </View>
              );
            }
            return null;
            ...
            <View key={index} style={styles.commentContainer}>
              ...
              {isAuthenticated && userEmails[index] === userEmail && (
                <TouchableOpacity style={styles.deleteButton} onPress={() => deleteComment(comment._id)}>
                  <Text style={styles.deleteButtonText} >Usuń komentarz</Text>
                </TouchableOpacity>
              )}
            </View>
          ))}
